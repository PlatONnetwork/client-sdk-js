...
class PlatonCommon {
    ...
    /**
     * @description 发交易中data的gas值的动态消耗
     */
    getTransactionDataPrice(str) {
        let dataArr = str.substring(2).split('').filter(function(x) {
            return x == '0';
        });
        let resGas = (str.length - dataArr.length - 2) * 68 + dataArr.length * 4;
        return resGas;
    }
